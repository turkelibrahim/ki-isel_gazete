...
const { EVENT_ADAPTER_SOURCES, EVENT_FEEDS, SMART_EVENT_SOURCES, EVENT_CATEGORY_MAP, getEventSourceSummary } = require("./eventSources");
...
const adapters = [
  ...
  require("./sources/curatedEventSourcesAdapter"),
];
...
function categoryAliasesFor(value = "") {
  const normalized = normalizeText(value);
  if (!normalized || normalized === "tumu" || normalized === "all") return [];
  const canonical = Object.entries(EVENT_CATEGORY_MAP || {}).find(([key, aliases]) => {
    const all = [key, ...(aliases || [])].map(normalizeText);
    return all.includes(normalized);
  });
  return canonical ? [normalizeText(canonical[0]), ...canonical[1].map(normalizeText)] : [normalized];
}

function applyFilters(events, filters = {}) {
  const city = normalizeText(filters.city || "");
  const category = String(filters.category || filters.type || "Tümü");
  const categoryAliases = categoryAliasesFor(category);
  ...
  return events.filter((event) => {
    if (city && city !== "turkiye" && city !== "türkiye" && city !== "all" && !normalizeText(event.city).includes(city)) return false;
    if (categoryAliases.length) {
      const haystackCategory = normalizeText(`${event.category} ${(event.tags || []).join(" ")}`);
      if (!categoryAliases.some((alias) => haystackCategory.includes(alias))) return false;
    }
...
function resolveDateRange(label, from, to) {
  ...
  if (value === "Tümü" || value === "Tumü" || value === "all") return { from: null, to: null };
  if (value === "Bugün") { end.setDate(start.getDate() + 1); return { from: start, to: end }; }
  if (value === "Yarın") { const tomorrow = new Date(start); tomorrow.setDate(start.getDate() + 1); const dayAfter = new Date(tomorrow); dayAfter.setDate(tomorrow.getDate() + 1); return { from: tomorrow, to: dayAfter }; }
...
function paginate(events, options = {}, sourceErrors = []) {
  ...
  const sourceNames = [...new Set(SMART_EVENT_SOURCES.filter((item) => item.enabled !== false).map((item) => item.name))];
  return {
    provider: "multi-source-adapter",
    cache: { ttlMs: CACHE_TTL_MS, cachedAt: cachedAt ? new Date(cachedAt).toISOString() : null },
    sourceSummary: getEventSourceSummary(),
    filters: {
      cities: ["ISTANBUL", "ANKARA", "IZMIR", "BURSA", "ANTALYA", "MUGLA", "ESKISEHIR", "KOCAELI", "TURKIYE"],
      types: ["Tümü", "Konser", "Tiyatro", "Festival", "Sergi", "Atölye", "Fuar", "Spor", "Çocuk", "Söyleşi", "Kültür Sanat", "Stand-up", "Etkinlik"],
      sources: ["Tüm Kaynaklar", ...sourceNames]
    },
    total: events.length,
    page,
    limit,
